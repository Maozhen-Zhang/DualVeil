...
class FLTrain():
    ...
    def metric(self, e):
        ...
        # if e == 1 or e == 99:
        #     Helper.metric(self.cfg, model, test_dataloder,
        #                   epoch=e
        #                   )
    ...
    def metricbackdoor(self, e, clients, choice_id, mal_id):
        ...
        if cfg.attack == "neurotoxin":
            ...
            asr, loss_asr, correct_asr, datasize_asr = metric(cfg, model, test_dataloder, IsBackdoor=True, epoch = 99)
            ...
            # current_lr = self.cosine_annealing_lr(e)
            # cfg.lr = current_lr
            # if cfg.n_client == 20:
    ...
    def cosine_annealing_lr(self, epoch):
        lr_max = 0.01  # 初始学习率
        lr_min = 0.0001  # 最小学习率
        max_epochs = 200  # 训练总 epoch 数
        """手动实现余弦退火学习率"""
        return lr_min + 0.5 * (lr_max - lr_min) * (1 + math.cos(epoch / max_epochs * math.pi))
