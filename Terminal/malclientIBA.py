...
class MalClientIBA(MalClient):
    ...
    def metric(self, cfg, model, dataloader, IsBackdoor=False, epoch=None):
        device = cfg.device
        model.eval()
                    ...
                    if epoch is not None:
                        for i, img in enumerate(ori_data):
                            ...
                            torchvision.utils.save_image(img,
                                                         f"visual/{cfg.attack}/{cfg.attack}-epoch_{epoch}-{batch_id}-{i}-ori.png")
                        for i, img in enumerate(data):
                            os.makedirs(f"visual/{cfg.attack}", exist_ok=True)
                            torchvision.utils.save_image(img,
                                                         f"visual/{cfg.attack}/{cfg.attack}-epoch_{epoch}-{batch_id}-{i}-poison.png")
